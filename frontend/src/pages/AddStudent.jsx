    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (isEditMode) {
                ...
                // ALWAYS show modal if parent credentials/info returned
                // Even if "Use Existing", showing the ID is helpful confirmation
        ...
        return (
            <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50">
                <div className="bg-white rounded-3xl p-8 max-w-sm w-full text-center">
                    <h2 className="text-2xl font-bold text-gray-800 mb-2">{formData.name} Added!</h2>
                    <p className="text-gray-500 mb-6 font-medium">Share these credentials with the parent</p>
                    ...
                    <div className="bg-gray-100 p-4 rounded-xl border border-gray-200 mb-6 text-left">
                        ...
                        {successCreds.password === '(Use Existing)' ? (
                            <p className="text-lg font-bold text-gray-600 italic">
                                Account Exists <br />
                                <span className="text-sm font-normal not-italic text-gray-500">(Ask parent to use their existing password)</span>
                            </p>
                        ) : (
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div>
                                ...
                                <select
                                    ...
                                    onChange={async (e) => {
                                        ...
                                        setFormData({ ...formData, classSection: section });

                                        // Auto-fetch next roll number
                                        if (section && !isEditMode) {
                                            try {
                                                const res = await axios.get(`${config.API_URL}/api/students/next-roll/${section}`);
                                                if (res.data.nextRoll) {
                                                    setFormData(prev => ({ ...prev, classSection: section, rollNumber: res.data.nextRoll.toString() }));
                                                }
                                            } catch (err) {
                        ...
                        <div className="flex gap-4 pt-4 border-t border-gray-100 mt-4">
                            ...
                            <button
                                type="submit"
                                disabled={loading}
                                className={`flex-1 p-4 bg-blue-600 text-white rounded-xl font-bold hover:bg-blue-700 transition shadow-lg shadow-blue-200 flex items-center justify-center gap-2 ${loading ? 'opacity-70 cursor-not-allowed' : ''}`}
                            >
                                {loading ? 'Saving...' : (
                                    <>
                                        <Save size={20} />
