...
const Login = () => {
  ...
  const [debugOtp, setDebugOtp] = useState(null); // Store debug OTP for display
  ...
  const handleLoginSubmit = async (e) => {
    ...
    try {
      ...
      if (res.data.step === 'OTP') {
        const otpDebug = res.data.debugOtp;
        if (otpDebug) alert("DEBUG MODE: Your OTP is " + otpDebug); // Show OTP in Alert as requested
       

        setStep('OTP');
        setUserEmail(res.data.email); // SAVE EMAIL 
  ...
  const handleOtpSubmit = async (e) => {
    ...
    if (!userEmail) {
      alert("System Error: Email lost. Please refresh and try again.");
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      console.error(err);
      gsap.fromTo(formRef.current, { x: -10 }, { x: 10, duration: 0.1, repeat: 5, yoyo: true });
      alert(`Debug Error: ${err.message}\n${err.response?.data?.message || "Check Network/Console"}`);
    } finally {
      setLoading(false);
    }
  };
              ...
              <>
                ...
                <div className="relative group">
                  <label className="flex justify-between text-sm font-medium text-gray-700 mb-1">
                    <span>Password</span>
                    <Link to="/forgot-password" className="text-green-600 hover:text-green-800 text-xs">Forgot Password?</Link>
                  </label>
                  <div className="relative">
