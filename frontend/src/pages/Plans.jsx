...
import { CheckCircle, Circle, ClipboardList, BookOpen, UserCheck, Calendar, Eye, X, Plus, AlertCircle, Trash2 } from 'lucide-react';
...
const Plans = () => {
    ...
    const handleDelete = async (id, e) => {
        ...
        try {
            const token = localStorage.getItem('token');
            await axios.delete(`${config.API_URL}/api/tasks/${id}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
            setTasks(tasks.filter(t => t._id !== id));
            if (selectedTask?._id === id) closeModal();
        } catch (err) {
