...
const Attendance = () => {
  ...
  useEffect(() => {
    ...
    const savedQueue = localStorage.getItem('offline_student_attendance');
  ...
  const syncOfflineData = async () => {
    const queue = JSON.parse(localStorage.getItem('offline_student_attendance') || '[]');
    if (queue.length === 0) return;

    const token = localStorage.getItem('token');
    const failed = [];

    alert(`Syncing ${queue.length} offline records...`);

    for (const req of queue) {
      try {
        await axios.post(`${config.API_URL}/api/students/attendance`, req, {
          headers: { Authorization: `Bearer ${token}` }
        });
      } catch (err) {
    ...
    if (failed.length === 0) {
      localStorage.removeItem('offline_student_attendance');
      setOfflineQueue([]);
      alert("✅ All offline records synced!");
    } else {
      localStorage.setItem('offline_student_attendance', JSON.stringify(failed));
      setOfflineQueue(failed);
      alert(`⚠️ Sync incomplete. ${failed.length} records remain offline.`);
    }
  };
  ...
  const toggleStatus = async (id, currentStatus) => {
    ...
    const payload = {
      studentId: id,
      status: newStatus,
      markedBy: userObj?._id || userObj?.id
    };

    try {
      const token = localStorage.getItem('token');
      // If we are already offline, skip straight to catch block (simulated) or just try axios
      if (!window.navigator.onLine) throw new Error("Offline");

      await axios.post(`${config.API_URL}/api/students/attendance`, payload, {
        headers: { Authorization: `Bearer ${token}` }
      });
    } catch (err) {
      console.log("Network failed, saving to offline queue");

      const currentQueue = JSON.parse(localStorage.getItem('offline_student_attendance') || '[]');
      const updatedQueue = [...currentQueue, payload];
      localStorage.setItem('offline_student_attendance', JSON.stringify(updatedQueue));
          ...
          <div className="p-6 flex flex-col md:flex-row justify-between items-center border-b border-gray-100 gap-4">
            <div>
              ...
              {offlineQueue.length > 0 && (
                <span className="inline-block mt-2 ml-2 px-3 py-1 bg-blue-100 text-blue-800 text-xs font-bold rounded-full">
